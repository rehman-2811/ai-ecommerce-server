...
const createOrder = async (req, res) => {
  try {
    ...
    if (paymentMethod === 'CARD') {
      return res.status(400).json({
        success: false,
        message: 'Card orders are placed automatically after payment succeeds — use /payments/card/create-intent instead.'
      });
    }
    ...
    logger.error('Create order error:', error);
    res.status(error.statusCode || 500).json({ success: false, message: error.message || 'Failed to create order' });
  }
};
